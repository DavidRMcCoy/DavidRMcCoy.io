...
The data set (too large to share on GitHub) includes 55,000 to 75,000 party lists per election year. The dependent variable is a percentage, which poses a challenge in terms of modeling. I treat each election as cross-sectional data. For each election, I run a generalized linear model that treats each single vote cast as a Bernoulli trial with "successful" trial being a vote cast for a female candidate. This is achieved by weighting each proportion value (observation outcome in the model) by the total number of votes gained by that party list. The quasi-binomial link function is similar to the binomial link used in logistic regression but includes an additional parameter that helps account for overdispersion in the data, otherwise the interpretation is the same. Below I run the four GLM models. 

## Weighted Binomial GLM Models

The main effects of interest are the interaction between party groups and municipality-level, future Bolsonaro support (the anti-system proxy). The interactions include polynomial terms in order to relax strict linearity assumptions, which makes interpretation of coefficients difficult in the table output. To enable interpretation, I visualize these interactions in the following code chunk. The other effects listed in the table are primarily included as controls for municipality-level factors and list-level factors.
...
These analysis of variance models show that increases in opposition to abortion rights are statistically significant from 2014 to 2018. The one-way model show (LHS plot below) shows a highly significant effect for `year` (the dummy for the legislature). However, the two-way model (RHS plot below) fails to reject the null that men and women increased their opposition in statistically significant levels. Both groups increased in their opposition similarly from 2014 to 2018. We discuss this in more detail in the book chapter.  
```{r, warning = FALSE, message = FALSE, results = "asis"}
# One-way ANOVA
