...
# Get time invariant vars from COVID data set to balance final set
id <- usc %>%
  group_by(place_id) %>%
  summarize(
...
3. Transformation $T$ for daily new deaths and daily new cases is the logged moving average of one plus daily counts per million:
$$T_{n,t} = (\frac{1}{7}) \sum_{i =n-3}^{n+3} p_{n-3}...p_{n+3}$$
Where $p_{n,t}$ equals the log of daily counts in county $n$ at time $t$ per million residents plus one:

$$p_{n,t} = log(1+(\frac{daily_{i,t}}{population_i} 1,000,000))$$
4. Seven-day moving average of Google Mobility index; same as $T_{n,t}$ but $p_{n,t}$ is the mean of the three standardized Google Mobility items at in county $n$ at time $t$.

Because a [reasonable estimate](https://www.thelancet.com/journals/lancet/article/PIIS0140-6736(20)30566-3/fulltext) of time between exposure and death is one month, I lag the Google Mobility index by 30 days. I lag new cases by 15 days. I should highlight that the relationships I expect to see in the data are exploratory and descriptive and cannot be expected to demonstrate causality in any direction. For this, I would need to go much further in eliminating endogeneity. I chose these three variables at these lagged points in time, because I am curious whether lagged community mobility and new cases are more associated with fewer deaths or whether this relationship is completely obscured (due to the endogenous correlation between past deaths with present deaths).
